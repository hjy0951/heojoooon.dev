...
title: "구조적 서브타이핑이란?"
description: "타입 시스템이 객체의 속성을 확인하는 방식"
tags: ["TypeScript"]
...
**타이핑**은 코드에서 값들의 **타입을 언제 결정하는지**를 말하는 것이었다.

예를 들면, `JavaScript`는 **런타임**에 타입이 결정되는 동적 타이핑 언어이다.
따라서 변수 선언시 따로 타입을 명시하지 않아도 런타임 과정에서 동적으로 해석한다.

반면 `TypeScript`는 **컴파일타임**에 타입이 결정되는 정적 타이핑 언어이다.
따라서 코드가 실행되기 이전에 타입을 먼저 검사하여 에러를 발견할 수 있는 것이다.

이와 다르게 **서브타이핑**은 값들의 타입 포함 관계, 즉, **상속 관계**를 나타내는 것이었다.
...
inTheForest 함수에서 파라미터의 타입을 `Quackable`로 정확히 명시하였다.
하지만, 가장 마지막줄에서 `Person`이라는 class는 `Quackable`을 상속 받지 않았지만 정상적으로 동작한다.
그 이유는 `Quackable` 인터페이스에 존재하는 quack이라는 함수가 `Person`에도 존재하기 때문에 같은 타입으로 간주되기 때문이다.
...
“명목적 서브타이핑”은 **타입 정의시 상속 관계으로 명시를 해야한 경우에만 타입 호환이 가능한 방식**이다.
...
“구조적 서브타이핑”은 **오직 객체 내의 멤버만으로 타입을 관계시키는 방식**이다.
...
아마 해당 내용에 대해 알아보다보면 덕 타이핑, 구조적 서브타이핑과 더불어 구조적 타이핑이란 용어도 등장하며 혼란스러워질 수 있다.
따라서 이를 다시 한번 나누어 이야기해보자면,

1. `덕 타이핑`은 동적 타입 언어에서 사용되는 방식으로, 런타임에 객체의 속성/메서드를 확인하여 타입을 결정한다.
2. `구조적 타이핑`은 정적 타입 언어에서 사용되는 방식으로, 컴파일에 객체의 구조를 검사하여 타입을 결정한다.
3. 마지막으로 `구조적 서브타이핑`은 구조적 타이핑의 확장된 개념으로, 더 구체적인 타입이 자동으로 서브타입으로 인정되는 집합 관계에 대한 개념이다.

따라서 최종적으로는,

- TypeScript에서 **“구조적 서브타이핑”** 은 핵심 규칙 중 하나이다.
- 구조적 서브타이핑은 **컴파일 타임**에 특정 값의 타입을 비교할 때, 명시된 이름이 아닌 구조, 즉, 속성과 메서드가 일치하는지 **모양을 비교하여 유연하게 타입 호환성을 제공**한다.

생각해보면 이러한 구조적 서브타이핑이 막상 장점만 존재할 것 같지는 않다.
선택적으로 속성이나 메서드가 존재할 수도 있고, 엄청 많은 인터페이스나 타입이 합쳐져 확장된 타입 또한 존재할 수 있을텐데, 이때 타입을 특정하거나 코드를 이해하기 힘들 수 있을 것 같다는 생각이 들었기 때문이다.