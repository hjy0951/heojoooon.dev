...
C를 쓰던 나에게는 큰 충격이였다.
C와 같은 `정적 타입 언어`는 **코드가 기계어로 컴파일될 때 타입이 정해진다.**
따라서 변수 선언 시에 타입을 같이 명시해주어야 한다.
따라서 컴파일 과정에서 타입을 비교하기 때문에 관련 오류를 미리 찾아낼 수 있다는 장점이 있다.

하지만 JavaScript는 var, let, const라는 키워드를 이용하여 변수를 선언하는 `정적 타입 언어`이다.
...
![javascript-runtime](/post-images/javascript/Javascript-Review/1.png)
