...
그럼 **방**에 해당하는 메모리 구조를 살펴보자.
다음은 JavaScript가 실행되는 엔진중, NodeJS와 크롬에서 사용하는 `V8 Engine`의 메모리 구조이다.

![V8 Engine의 메모리 구조](/post-images/develop/gc-in-JavaScript/1.png)

우선, JavaScript는 Call Stack이 하나인 **Single-Thread** 언어이기 때문에 하나의 `Stack`이 보인다.

그 외에는 `Heap`으로 Stack에 비해 구조가 복잡해보인다.
오늘 눈여겨 볼 공간은 <u>**Heap 영역**과 그 안에 존재하는 **New Space**와 **Old Space**</u>이다.

## JavaScript의 메모리 관리

그럼 다음은 방을 어질러 놓을 **Garbage**들이 어떻게 생기는지 알아보자.
JavaScript에서는 <u>변수 선언시 자동으로 메모리를 할당</u>한다.

### Stack 영역

number, string 등과 같은 **Primitive Type**의 경우, `Stack` 영역에 저장하고, 함수 호출이 끝나면 콜스택을 빠져나가며 OS에 의해 메모리가 정리된다.
...
<CallOut type="check">
이렇게 Garbage Collector가 사용하지 않는 메모리를 수거하고, 해제하는 것을 **"Garbage Collection"** 이라고 한다.
</CallOut>

**Deepu K Sasidharan**라는 분께서 만드신 [V8 Engine에서 메모리를 사용 동작에 대한 예시](https://speakerdeck.com/deepu105/v8-memory-usage-stack-and-heap) 슬라이드를 보고 이해하는데 많은 도움이 되어 링크를 공유해본다.
...
![Heap의 구조](/post-images/develop/gc-in-JavaScript/2.png)

우선, 이렇게 나뉘게 된 배경은 [The Generational Hypothesis](https://www.memorymanagement.org/glossary/g.html#term-generational-hypothesis)라는 가설이다.
(GPT피셜 **John McCarthy**라는 분이 주장했다고 함!)
해당 가설의 내용은 다음과 같다.

> 대부분의 경우, 새로운 객체가 오래된 객체보다 쓸모없어질 가능성이 높다.

가설에 따라보면, 모든 객체를 매번 검사하여 GC를 실행하기보다, 오래된 객체보다 새로운 객체들에 대해서 GC를 실행하는 것이 더 효율적일 것이다.
이러한 가설을 바탕으로 V8 Engine은 이러한 특성에 맞게 다음과 Heap 영역을 분리하였다.
...
<CallOut type="light">
Scavenger란 까마귀, 독수리, 하이에나 등과 같이 자연에서 청소부 역할을 하는 동물을 뜻함 - [나무위키](https://namu.wiki/w/%EC%8A%A4%EC%BA%90%EB%B9%88%EC%A0%80)
</CallOut>

**Scavenger**라 불리는 Minor GC는 New Space에서 청소를 담당한다.
Minor GC는 `Mark & Sweep` 알고리즘을 통해 살아남을 객체를 선별한다.
Minor GC는 매우 자주 발생하고, 병렬 헬퍼 스레드(Parallel Helper Thread)를 사용하기 때문에 굉장히 빠르다.

Minor GC에서 살아남은 객체들은 수거 대상들과 분류를 위해 항상 새로운 공간으로 대피를 해야한다.
...
- Garbage Collection의 대상이 되는 객체를 선별하기 위한 단계이다.
...
![mark-sweep-compact](/post-images/develop/gc-in-JavaScript/8.gif)
...
![tri-color](/post-images/develop/gc-in-JavaScript/5.png)
![tri-color](/post-images/develop/gc-in-JavaScript/6.png)
![tri-color](/post-images/develop/gc-in-JavaScript/7.png)
...
기존 GC는 Main Thread 혼자 수행했지만, 이를 Helper Thread들과 나누어 수행하도록 변경하였다.
Thread 간 동기화 문제를 처리하는 오버헤드가 발생하지만, STW 시간을 크게 줄였다.

![Parallel GC](/post-images/develop/gc-in-JavaScript/9.png)
...
![Incremental GC](/post-images/develop/gc-in-JavaScript/10.png)

### Concurrent

Main Thread에서는 GC를 수행하지 않고, 온전히 Helper Thread들이 GC를 수행한다.
STW가 사라지는 큰 장점이 있지만, 기술적으로 구현이 어렵다.

![Concurrent GC](/post-images/develop/gc-in-JavaScript/11.png)
...
<CallOut type="warn">
GC 이야기보다는 제 생각과 운영체제 개념들이 등장하여 넘어가셔도 좋습니다 :)
</CallOut>

다른 이야기긴 하지만, 이전 회사에서 나는 Linux 환경에서 모바일 기기의 Kernel Driver를 개발하는 업무를 진행했었다.
